...
function executeLine(cmd, args, line) {
  var bi = builtins[cmd] || operations[cmd];
  if ((ctx.client === undefined || !ctx.client.connected) && !bi.supportsNoConnection) {
...
function fillContextHome(callback) {
  if (operations['UserWorkspace.Get']) {
    var operation = ctx.client.operation('UserWorkspace.Get');
    ...
    console.warn("Unable to set User home path.");
    callback();
  }
}

function fillOperationsCommands(ops) {
  ops.forEach(function(op) {
    ...
    var needInput = false,
      acceptVoid = false;
    for (var i = 0; i < op.signature.length; i += 2) {
      if (op.signature[i] === "void") {
        acceptVoid = true;
      } else {
        needInput = true;
      }
    ...
    var impl = (function(id) {
      return function(args) {
        ...
        operations[id].options.forEach(function(opt) {
...
var builtins = {},
  ...
  completer = function(line, callback) {
    ...
    if ((cmds.length > 0 && line[line.length - 1] === ' ') || cmds.length > 1) {
      var b = builtins[cmd] || operations[cmd];
      if (lastCmd && lastCmd.indexOf("-") == 0) {
        ...
        // check if a completer exists
        b.completer(line, args, callback);
      } else {
      ...
      b = b.concat(Object.keys(operations));
      var hits = b.filter(function(c) {
...
var prettyPrinter = (function(pp) {
  ...
  pp.group = function(group) {
    console.log(JSON.stringify(group, null, 2));
  };

  pp.groups = function(data) {
    print.pt(data.entries);
  };
...
rl.on("line", function(line) {
  ...
  if (typeof builtins[cmd] === "object" || typeof operations[cmd] === "object") {
    executeLine(cmd, args, line);
  } else {
    var message = pjson.name + ": command not found: " + cmd;
...
builtins.commands = {
  impl: function() {
    for (var name in builtins) {
      console.log(name.yellow + " - " + builtins[name].help || "");
    }
    prompt();
  },
  supportsNoConnection: true,
  help: "List existing commands."
};

builtins.operations = {
  impl: function() {
    for (var name in operations) {
      console.log(name.yellow + " - " + operations[name].help || "");
    }
    prompt();
  },
  supportsNoConnection: true,
  help: "List existing operations."
};
...
builtins.cd = {
  impl: function(args) {
    ...
    // homePath Replace
    if (path.indexOf("~") === 0 && ctx.home) {
      // replace home path
      path = p.join(ctx.home, path.substring(1, path.length));
    }
  ...
  completer: function(line, args, callback) {
    var fullPath = args[0] || "";
    if (!p.isAbsolute(fullPath)) {
      fullPath = p.join(ctx.path, fullPath);
    }

    var name = p.basename(fullPath);
    var path = p.dirname(fullPath);
    if (args[0] === undefined || fullPath[fullPath.length - 1] === "/") {
      name = undefined;
      path = fullPath;
    }

    var q = "SELECT * FROM Document WHERE ecm:path STARTSWITH '" + path + "' AND ecm:isCheckedInVersion = 0";
    if (name !== undefined) {
      q += " AND ecm:name LIKE '" + name + "%'";
    }
    q += " ORDER BY ecm:path";
    var operation = ctx.client.operation("Document.PageProvider");
    operation.param("query", q);
    ...
    operation.execute(function(error, data, response) {
      ...
      data.entries.forEach(function(entry) {
        var parentPath = path !== "/" ? path + "/" : path;
        var finalPath = entry.path.replace(parentPath, "");
        if (finalPath.indexOf("/") === -1) {
          var n = p.basename(entry.path);
          n = n.replace(/ /g, '\\ ');
          suggestions.push(n);
        }
      });
...
builtins.touch = {
  impl: function(args) {
    if (!args.args[0]) {
      console.log("Usage: touch <filename>".red);
      prompt();
      return;
    }
...
builtins.groupshow = {
  impl: function(args) {
    ...
    ctx.client.request("group").path(args.args[0]).get(function (error, data, response) {
      if (error) {
        console.log(error);
        prompt();
        return;
