...
builtins.get = {
  impl: function(args) {
    ...
    if (args.args.length >= 2) {
      ...
      remotePath = remotePath[0] === "/" ? remotePath : ctx.path + "/" + remotePath;
      localPath = args.args[1];
      localPath = localPath[0] === "/" ? localPath : __dirname + "/" + localPath;
    ...
    operation.execute({ decoding: "buffer" }, function(error, data, response) {
      ...
      fs.writeFile(path.join(localPath, filename), data, function(error) {
        if (error) {
  ...
  options: [{
    short: "i",
    long: "id",
    desc: "Receive a Blob of a specific document using his uid."
  },{
    short: "p",
    long: "property",
