  ...
  impl: function(args) {
    ...
    ctx.client.request("path").path(path).get({
        ...
        if (doc["entity-type"] === "document") {
          ...
          rl.setPrompt(doc.path.green + " > ".yellow, t.length - 20);
        } else {
...
builtins.mkdir = {
  ...
  options: [{
    ...
    short: "p",
    long: "properties",
    desc: "Set basic metadata. Format: -p dc:title=asdad,dc:description=afsdfsdf."
  }]
};

builtins.cat = {
  impl: function(args) {
    var request = ctx.client.request("id");
    if (args.id) {
      request.path(args.id);
    } else {
      request = ctx.client.request("path");
      if (!args.args[0]) {
        request.path(ctx.doc.path)
      } else {
        var path = args.args[0][0] === "/" ? args.args[0] : ctx.path + "/" + args.args[0];
        request.path(path)
      }
    }
