...
function executeLine(cmd, args) {
  ...
  bi.impl(parser.parse(["node"].concat(args)));
}
    ...
    if (cmds.length > 0) {
      ...
      if (cmds[cmds.length - 1].indexOf("-") == 0) {
        ...
        var options = b.options || [],
          option = splitArgs(line).pop().replace(/^-*/, ""),
          hits = options.filter(function(o) {
            return (o.short || "").indexOf(option) == 0 || (o.long || "").indexOf(option) == 0;
          });
...
builtins.userdel = {
  impl: function(args) {
    ...
    rl.question("Remove " + username + "? ", function(input) {
      if (input.indexOf("y") == 0) {
        ctx.client.request("user").path(username).delete({
          done: function() {
            prompt();
          },
          fail: function(data) {
...
builtins.groupshow = {
  impl: function(args) {
    ...
    var request = ctx.client.request("group").path(args.args[0]).get({
      done: function(data) {
        prettyPrinter.print(data);
        prompt();
      }
...
builtins.groupdel = {
  impl: function(args) {
    ...
    rl.question("Remove " + groupname + "? ", function(input) {
      if (input.indexOf("y") == 0) {
        ctx.client.request("group").path(groupname).delete({
          done: function() {
            prompt();
          },
          fail: function(data) {
